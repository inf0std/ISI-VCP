...
const createReunion = async (
  ...
  return reunion
    ...
    .then(async (Reunions) => {
      ...
      User.updateMany(
        { $or: [{ _id: { $in: Reunions.participantsName } }, { _id: idU }] },
        { $push: { reunions: newreunionId } }
      ).then((user) => {
        console.log({
          message: `${user.modifiedCount} updated successfully!`,
        });
    ...
    .catch((err) => {
      return console.log({
        message:
          err.message || "Some error occurred while creating the reunion.",
      });
    });
};
// désigner le modirateur
const Moderateur = async (idR, idM) => {
  //idR = id of reunion     // idM =  id of the new moderateur
  //const id = req.params.id;

  return Reunion.updateOne({ _id: idR }, { reunion_moderateur: idM }).then(
    (user) => {
      return {
...
const readReunion = async (id) => {
  ...
  Reunion.findById(id)
    .then(async (Reunion) => {
      if (!Reunion) console.log({ message: "Not found Reunion with id " + id });
      else console.log(Reunion);
      return Reunion;
    })
    .catch((err) => {
      console.log({ message: "Error retrieving Reunion with id = " + id });
    });
};
...
const deleteReunionAll = async () => {
  return Reunion.countDocuments({ archive: false }).then((reunion) => {
    if (reunion > 0)
      Reunion.updateMany({ archive: true }).then(async (data) => {
        console.log({
